//! Finding synthesis: convert anomalies, health metrics, and structural
//! diagnostics into prioritized, developer-facing issues.
//!
//! Ports logic from Python topo_analyzer.analysis._build_findings,
...
pub fn build_findings(ctx: &FindingsContext) -> Vec<IssueOutput> {
    let mut findings: Vec<IssueOutput> = Vec::new();

    // 1. Coverage issue.
    if ctx.spectral_coverage_ratio < 0.75 {
        findings.push(IssueOutput {
    ...
    findings.extend(detect_reverse_dependencies(ctx));

    // 5. Anomaly → finding conversion.
    let suppress_outliers = ctx.largest_module_ratio >= 0.8
        || ctx
            .silhouette
            .map(|s| s < 0.3)
            .unwrap_or(false);
    ...
    let role_weights: HashMap<&str, f64> = [
        ("hub", 1.5),
        ("bridge", 1.3),
        ("entry_point", 1.2),
        ("utility", 1.1),
    ...
    for anomaly in ctx.anomalies {
        if anomaly.kind == AnomalyKind::CrossModule {
            continue;
        }
        ...
        if anomaly.kind == AnomalyKind::SpectralOutlier {
            if let Some(nid) = anomaly.node_ids.first() {
                if let Some(&role) = role_map.get(nid.as_str()) {
                    if let Some(&weight) = role_weights.get(role) {
    ...
    // 6. Orphan findings.
    findings.extend(detect_orphan_findings(ctx));

    // 7. God module detection.
    if !suppress_outliers {
        findings.extend(detect_god_modules(ctx));
    }

    // 8. Low cohesion.
    findings.extend(detect_low_cohesion(ctx));

    // 9. Fragile hub.
    findings.extend(detect_fragile_hubs(ctx));

    // 10. Wide interface.
    findings.extend(detect_wide_interfaces(ctx));

    // 11. Phantom imports.
    findings.extend(detect_phantom_imports(ctx));
...
fn detect_reverse_dependencies(ctx: &FindingsContext) -> Vec<IssueOutput> {
...
fn detect_orphan_findings(ctx: &FindingsContext) -> Vec<IssueOutput> {
...
fn detect_god_modules(ctx: &FindingsContext) -> Vec<IssueOutput> {
    let clustered: Vec<&EnrichedModule> = ctx.modules.iter().filter(|m| !m.unassigned).collect();
    if clustered.len() < 2 {
        return Vec::new();
    }

    let mut sizes: Vec<usize> = clustered.iter().map(|m| m.node_ids.len()).collect();
    sizes.sort();
    let median_size = sizes[(sizes.len() - 1) / 2];
    let total_edges = ctx.graph.edge_count;
    let k = clustered.len();
    let fair_share = 2.0 / k as f64;
    ...
    for m in &clustered {
        ...
        let edge_count: usize = ctx
            ...
            .count();
        let edge_share = edge_count as f64 / total_edges.max(1) as f64;

        if edge_share > 2.0 * fair_share || m.node_ids.len() > 3 * median_size {
            let sev = (0.4 + edge_share).min(1.0);
            findings.push(IssueOutput {
...
fn detect_low_cohesion(ctx: &FindingsContext) -> Vec<IssueOutput> {
    let mut findings = Vec::new();
...
fn detect_fragile_hubs(ctx: &FindingsContext) -> Vec<IssueOutput> {
...
fn detect_wide_interfaces(ctx: &FindingsContext) -> Vec<IssueOutput> {
    let clustered: Vec<&EnrichedModule> = ctx.modules.iter().filter(|m| !m.unassigned).collect();
    if clustered.len() < 2 {
        return Vec::new();
    }
    ...
    let mut widths: Vec<usize> = pair_symbols.values().map(|s| s.len()).collect();
    widths.sort();
    let median_width = widths[(widths.len() - 1) / 2];
    let threshold = (2 * median_width).max(4);
    ...
    for ((mod_a, mod_b), syms) in &pair_symbols {
        ...
        let label_b = module_by_id
            ...
            .map(|m| m.label.as_str())
            .unwrap_or("?");
        let sev = (0.3 + (width - median_width) as f64 / width.max(1) as f64 * 0.7).min(1.0);
...
fn detect_phantom_imports(ctx: &FindingsContext) -> Vec<IssueOutput> {
    let clustered: Vec<&EnrichedModule> = ctx.modules.iter().filter(|m| !m.unassigned).collect();
    if clustered.len() < 2 {
        return Vec::new();
    }
