...
from topo_parser_python.ast_resolve import (
...
def _extract_from_file(
    ...
    # Module end line: last line of the file
    mod_line_end = tree.body[-1].end_lineno if tree.body and hasattr(tree.body[-1], "end_lineno") else None
    graph.add_node(Node(id=mod_id, kind=NodeKind.MODULE, file=str(file), line=1, name=mod_id.split(".")[-1], line_end=mod_line_end))
...
def _extract_function(
    ...
    graph.add_node(Node(id=func_id, kind=NodeKind.FUNCTION, file=str(file), line=node.lineno, name=node.name, line_end=getattr(node, "end_lineno", None)))
