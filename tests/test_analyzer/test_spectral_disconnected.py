...
def test_disconnected_spectral_marks_small_components_unassigned():
    ...
    for name in ["a0", "a1", "a2", "b0", "b1"]:
        graph.add_node(Node(id=name, kind=NodeKind.FUNCTION, file="m.py", line=1, name=name))
...
def test_fingerprint_width_consistent_across_components():
    ...
    for i in range(5):
        graph.add_node(Node(id=f"big.{i}", kind=NodeKind.FUNCTION, file="m.py", line=1, name=f"b{i}"))
    for i in range(4):
        graph.add_edge(Edge(source=f"big.{i}", target=f"big.{i+1}", kind=EdgeKind.CALLS))
    # Component 2: 3 nodes (will get k=1 eigenvector)
    for i in range(3):
        graph.add_node(Node(id=f"sm.{i}", kind=NodeKind.FUNCTION, file="m.py", line=1, name=f"s{i}"))
